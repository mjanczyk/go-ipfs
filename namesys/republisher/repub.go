...
func (rp *Republisher) republishEntries(p goprocess.Process) error {
	...
	for id, _ := range rp.entries {
		...
		namekey, ipnskey := namesys.IpnsKeysForID(pkb)
		...
		entry, err := namesys.CreateRoutingEntryData(priv, p, e.GetSequence())
		if err != nil {
			return err
		}

		// republish public key
		err = rp.r.PutValue(ctx, namekey, pkb)
		if err != nil {
			return err
		}

		// republish ipns entry
		err = rp.r.PutValue(ctx, ipnskey, entry)
		if err != nil {
			return err
		}
	}
